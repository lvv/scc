...
// SEQUANCE CONTAINTER or C-ARRAY
	...
	eIF<is_iterable<Ct>()  &&  !is_ioable_t<Ct>::value, std::ostream&>
...
struct in_t {
	...
	// SEQ-CONT
		template<typename Ct>
		eIF<is_iterable<Ct>()  &&  has_push_back<Ct>()  &&  !is_ioable_t<Ct>::value,  void>
	input(Ct& C)	{
	...
	// SET
		template<typename Ct>
		eIF<is_iterable<Ct>()  &&  has_1arg_insert<Ct>()  &&  !has_mapped_type<Ct>(),  void>
	input(Ct& C)	{
		typename Ct::value_type t;
		C.clear(); 
		n = n ? n : -1;
...
// SEQ-CONTAINER
		...
		template<typename Ct>
		eIF<is_iterable<Ct>()  &&  has_push_back<Ct>()  &&  !is_ioable_t<Ct>::value, std::istream& >
...
// SET
		template<typename Ct>
		eIF<is_iterable<Ct>()  &&  has_1arg_insert<Ct>() && !has_mapped_type<Ct>(), std::istream& >
operator>>      (std::istream& is, Ct& C)    {
	typename Ct::value_type c;
